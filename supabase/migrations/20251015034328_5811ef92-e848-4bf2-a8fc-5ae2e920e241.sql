-- Fix api_rate_and_review to use user_reviews table correctly
...
begin
  ...
  if p_review is not null and length(trim(p_review)) > 0 then
    -- Insert into user_reviews (upsert to handle duplicates)
    ...
    values (auth.uid(), 'review', p_review, p_item_type, p_item_id, v_score, p_is_spoiler)
    returning id into v_post_id;
  elsif v_score is not null then
    -- rating-only activity post
    insert into posts(author_id, kind, body, item_type, item_id, rating_percent, is_spoiler)
    values (auth.uid(), 'rating', null, p_item_type, p_item_id, v_score, p_is_spoiler)
    returning id into v_post_id;
  end if;
